...
const processInput = (input: unknown) => {
  if (typeof input === "string") {
    ...
    if (Number.isInteger(input)) console.log(`Đây là một số nguyên: ${input}`);
    else console.log(`Đây là một số thực: ${input.toFixed(2)}`);
...
const processInputAny = (input: any) => {
  // Any quên dùng IF để kiểm tra, cứ thế gọi hàm luôn
